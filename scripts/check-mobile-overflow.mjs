...
const checks = [
  ...
  ["mobile feature cards constrained two columns", /@media\s*\(max-width:\s*720px\)[\s\S]*?\.etf-feature-groups,[\s\S]*?\.etf-decision-grid,[\s\S]*?\.etf-monitor-grid,[\s\S]*?\.etf-reserved-grid\s*{[\s\S]*?max-width:\s*100%[\s\S]*?width:\s*100%[\s\S]*?@media\s*\(max-width:\s*540px\)[\s\S]*?grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*calc\(\(100% - 8px\) \/ 2\)\)\)/],

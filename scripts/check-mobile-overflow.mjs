...
const checks = [
  ...
  ["mobile feature cards flattened two columns", /@media\s*\(max-width:\s*720px\)[\s\S]*?\.etf-feature-groups\s*{[\s\S]*?display:\s*grid[\s\S]*?grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)[\s\S]*?\.etf-tool-section-label\s*{[\s\S]*?grid-column:\s*1\s*\/\s*-1[\s\S]*?\.etf-decision-grid,[\s\S]*?\.etf-monitor-grid,[\s\S]*?\.etf-reserved-grid\s*{[\s\S]*?display:\s*contents/],
